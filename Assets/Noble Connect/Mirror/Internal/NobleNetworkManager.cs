...
namespace NobleConnect.Mirror
{
    public class NobleNetworkManager : NetworkManager
    {
        ...
        override public void Awake()
        {
            ...
            bool hasUDPTransport = MirrorHelper.HasUDPTransport();

            if (!hasUDPTransport)
            {
                Debug.LogWarning(TRANSPORT_WARNING_MESSAGE);
            }
        }
        ...
        new public void StartClient()
        {
            ...
            bool isAddress = IPAddress.TryParse(networkAddress, out relayAddress);
            if (!isAddress)
            {
                throw new Exception("The supplied networkAddress is not a valid IP address.");
            }
            var relayEndPoint = new IPEndPoint(relayAddress, networkPort);
        ...
        new virtual public void StartHost()
        {
            ...
            MirrorHelper.SetTransportPort((ushort)networkPort);
        ...
        public void RegisterClientHandlers()
        {
            ...
            // Mirror's network manager will have set the OnDisconnected event
            // to an internal method, but we need those events to call the methods they are hooked up
        ...
        override public void OnStartServer()
        {
            ...
            if (!isLANOnly)
            {
                try
                {
                    ...
                    ushort port = MirrorHelper.GetTransportPort();
        ...
        public void OnStartServerLANOnly()
        {
            ...
            networkPort = MirrorHelper.GetTransportPort();
        ...
        /// </remarks>
        virtual public void OnFatalError(Exception e)
        {
        ...
        virtual public void OnClientError(Exception exception)
        {
            Debug.LogError("Fatal client error: " + exception + "\n" + exception.StackTrace);
            StopClient();
        }

        virtual public void OnServerError(Exception exception)
        {
            Debug.LogError("Fatal server error: " + exception + "\n" + exception.StackTrace);
            StopHost();
        }
        ...
        {
            base.OnDestroy();
            // Only Dispose static NobleServer if this is "main" network manager
