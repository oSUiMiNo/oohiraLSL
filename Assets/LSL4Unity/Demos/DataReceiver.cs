...
public class DataReceiver : ADoubleInlet {
    ...
    ///// number of channels
    ///// </summary>
    //public string NumChans { get; private set; }
    ...
    ///// header of data
    ///// </summary>
    //public string DataHeaderTxt { get; private set; }

    ///// <summary>
    ///// data stream
    ///// </summary>
    //public string DataStreamTxt { get; private set; }
        ...
        if (listStreams.Count > 0) {
            ...
            foreach ( var stream in QueryAvailStreams()) {
                ...
                if (name == _currStreamName) {
                    DeviceID.Value    = stream.source_id();
                    NumChans.Value    = stream.channel_count().ToString();
                    return;
                }
            }
        }
    ...
    public void OnConnectClick() {
        ...
        if (!string.IsNullOrEmpty(info.name())) {
            ...
            if (_inletCreated) {
                ...
                DeviceID.Value    = info.source_id();
                NumChans.Value    = info.channel_count().ToString();
                DataHeaderTxt.Value = string.Join("; ", GetChannelsList().ToArray());
    ...
    {
        // Show data on UI
        DataStreamTxt.Value = string.Join("; ", newSample);
    }
